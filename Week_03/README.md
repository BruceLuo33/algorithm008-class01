...
```
- 从上面的数组，我们可以很明显的看到，假设数组长度无限，那么每一个数组其实都全部包含了余下的两个数组。那么我们要做的就很简单了，设置三个指针，分别指向这三个数组的某一个位置，然后乘以相应的系数，判断三者的相对大小。将更小的数放入我们的丑数数组中，依次循环。举例来说：
```Java
Step 1:
Array 1: 1x2, 2x2, 3x2, 4x2, 5x2 ……
           ^
...
Array 3: 1x5, 2x5, 3x5, 4x5, 5x5 ……
           ^
UglyNumber[0] = 1, UglyNumber[1] = min{1x2, 1x3, 1x5} = 2. 因此将 Array 1 的指针往后移，其余两个保持不动
...
Array 3: 1x5, 2x5, 3x5, 4x5, 5x5 ……
           ^
UglyNumber[0] = 1, UglyNumber[1] = 2，UglyNumber[2] = min{2x2, 1x3, 1x5} = 3. 将 Array 2 的指针后移。以此类推，直到我们到达第 k 个数
