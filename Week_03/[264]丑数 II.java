...
class Solution {
    public int nthUglyNumber(int n) {
        ...
        ans[0] = 1;
        int product2 = 0, product3 = 0, product5 = 0;
        int index2 = 0, index3 = 0, index5 = 0;
        for (int i = 1; i < n; i++) {
            product2 = 2 * ans[index2];
            product3 = 3 * ans[index3];
            product5 = 5 * ans[index5];
            ans[i] = Math.min(Math.min(product2, product3), product5);
            if (ans[i] == product2) index2 += 1;
            if (ans[i] == product3) index3 += 1;
            if (ans[i] == product5) index5 += 1;
        }
        return ans[n-1];
